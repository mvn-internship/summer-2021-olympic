...
@section('content')
    @if (is_null($tournamentEdit))
        <div class="alert alert-danger" role="alert">
            {{ __('label.this_tournament_not_exist') }}
        </div>
    @else
        <div class="card shadow mb-4">
            <div class="card-header d-flex justify-content-between align-items-center py-3">
                <h6 class="m-0 font-weight-bold text-primary inline-flex">{{ __('label.edit_tournament') }}</h6>
            </div>
            <div class="card-body">
                <form method="PUT" id="editTournament">
                @csrf
                @method('PUT')
                    <!-- Form Row-->
                    <div class="row gx-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input class="form-control" id="name" name="name" type="text" placeholder="Enter user name" value="{{ $tournamentEdit->name }}">
                            <input class="form-control" id="id" name="id" type="text" value="{{ $tournamentEdit->id }}" hidden>
                    ...
                    <div class="row gx-3 mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" id="startDate" name="startDate" class="form-control" value="{{ date('m/d/Y', strtotime($tournamentEdit->start_date)) }}" autocomplete="off">
                            <span id="startDateError"></span>
                        </div>
                        <div class="col-md-6">
                            <label class="required" for="password">{{ __('label.end_date') }}</label>
                            <input type="text" id="endDate" name="endDate" class="form-control" value="{{ date('m/d/Y', strtotime($tournamentEdit->end_date)) }}" autocomplete="off">
                        ...
                        <div class="col-md-6">
                            <select id="team" name="team" multiple="multiple" hidden>
                                @foreach($tournamentEdit->organizationTournaments as $organizationTournament)
                                    <option value="{{ $organizationTournament->team->id }}" selected>{{ $organizationTournament->team->name }}</option>
                                @endforeach
...
@section('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#editTournament").submit(function (event) {
                var data = {
                    name: $("#name").val(),
